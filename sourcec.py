...
def rand_checkers(n1=100,n2=100,epsilon=0.1):
    ...
    yapp=[1]*((nbp+nbn)*8)
    idx=0
    for i in list(range(-2,2)):
        for j in list(range(-2,2)):
            if (((i+j) % 2)==0):
                nb=nbp
            else:
                nb=nbn
...
def plot_2d(data,y=None,w=None,alpha_choice=1):
    ...
    if (y is not None): 
        labs=np.unique(y)
        idxbyclass=[ np.where(y==labs[i])[0] for i in list(range(len(labs)))]
    else :
        labs=[""]
        idxbyclass=[range(data.shape[0])]

    #Plot ...
    for i in list(range(len(labs))):
...
def gradient(x,y,epsilon,niter,w_ini,lfun,gr_lfun,stoch=True):
    ...
    for i in range(niter):
        ...
        #loss[i]=lfun(x[idx],y[idx],w[i,:]).mean()
    return [w,loss]
